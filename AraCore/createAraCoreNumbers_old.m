...
%unit in the model is mmol/gdw(/hr) while literature is given in umol/gdw
species_sim_conc = species_lit_conc;
for i= 1:length(species_lit_id)
    species_sim_conc(i) = 1000*sum(last_met_throughput(mets_no_cmp == species_lit_id(i)));
end
species_sim_conc(species_sim_conc == 0)=min(species_sim_conc(species_sim_conc>0));
[rho,pval] = corr(species_lit_conc,species_sim_conc, 'Type', 'Pearson');
[rho_log,pval_log] = corr(log10(species_lit_conc),log10(species_sim_conc), 'Type', 'Pearson');
[rho_sp,pval_sp] = corr(species_lit_conc,species_sim_conc, 'Type', 'Spearman');
scatter(log10(species_sim_conc),log10(species_lit_conc));
scatter(1:length(species_lit_conc), sort(log10(species_lit_conc./species_sim_conc)));

species_lit_aa_idx = find(ismember(species_lit_id,amino_acids));
species_lit_aa_id = species_lit_id(species_lit_aa_idx);
species_lit_aa_conc = species_lit_conc(species_lit_aa_idx);
species_sim_aa_conc = species_sim_conc(species_lit_aa_idx);

filtered_aa = find(species_sim_aa_conc);
[rho_aa,pval_aa] = corr(species_lit_aa_conc(filtered_aa),species_sim_aa_conc(filtered_aa),'Type','Pearson');
[rho_aa_log,pval_aa_log] = corr(log10(species_lit_aa_conc(filtered_aa)),log10(species_sim_aa_conc(filtered_aa)),'Type','Pearson');
[rho_aa_sp,pval_aa_sp] = corr(species_lit_aa_conc(filtered_aa),species_sim_aa_conc(filtered_aa),'Type','Spearman');
scatter((species_sim_aa_conc(filtered_aa)),(species_lit_aa_conc(filtered_aa)));
figure;
grid on
[~,aa_conc_ratio_sorted_indices] = sort(log10(species_lit_aa_conc./species_sim_aa_conc));
%scatter(1:length(species_lit_aa_conc(filtered_aa)), (log10(species_lit_aa_conc(filtered_aa)./species_sim_aa_conc(filtered_aa))));
scatter(1:length(species_lit_aa_conc), (log10(species_lit_aa_conc(aa_conc_ratio_sorted_indices)./species_sim_aa_conc(aa_conc_ratio_sorted_indices))));
ylabel("log10(lit. conc. / sim. conc.)");
xlim([0 21])
xticks(1:20);
%xticklabels(species_lit_aa_id(filtered_aa));
xticklabels(species_lit_aa_id(aa_conc_ratio_sorted_indices));

figure;
grid off

simFluxes = solution.v*2; %scale it to 6mgdw/gdw*h growth and scale tu umol/gdw*h fluxes
simFluxes = simFluxes/12; %per 5min. instead of 1h
...
relevant_pools = ismember(mets_no_cmp, N_species_id);

history_length = 240;
history_points = [25, 61, 121, 241];
[history_ratio, hist_N14, hist_N15] = simMFA(model, simFluxes, history_length, atom_names, atom_map_rxns, ...
    atom_map_mapping, N15_feed_rxn_ids, N15_feed_atoms, met_pools, 150000, model_bio1_rxn_index);

if 0
    figure2 = figure;
    histogram(log(history_ratio(history_ratio(:,(history_length+1))<=0.1,(history_length+1)))/log(10),100)
    title('Logarithmic histogram of metabolic pools with N15 ratio of <=10%');
    ...
    % Create histogram
    histogram(log(history_ratio(history_ratio(:,(history_length+1))<=0.1,(history_length+1)))/log(10),'Parent',axes1,'BinEdges',-23:-1);
    title('Logarithmic histogram of metabolic pools with N15 ratio of <=10%');
    ...
    set(axes1,'XTick',[-20,-15,-10,-5]);
    set(axes1,'XTickLabel',{'10^{-20}','10^{-15}','10^{-10}','10^{-5}'});
end
...
history_ratio2 = simMFA(model2, solution2.v, history_length, atom_names, ...
    atom_map_rxns, atom_map_mapping, N15_feed_rxn_ids, N15_feed_atoms);


for history_point=history_points
    f = figure;
    histogram(history_ratio(history_ratio(:,(history_length+1))>0.01,(history_point)),(0:100)*0.01)
    title(sprintf('Histogram of metabolic pools with N15 ratio of >0.01, after %d h, with opt. Biomass', (history_point-1)/10));
    ylim([0,80]);
    xlabel('N15 ratio');
    f.Position(3:4)=[960,400];
    f = figure;
    histogram(history_ratio2(history_ratio2(:,(history_length+1))>0.01,(history_point)),(0:100)*0.01)
    title(sprintf('Histogram of metabolic pools with N15 ratio of >0.01, after %d h with N-limited Biomass', (history_point-1)/10));
    ylim([0,80]);
    xlabel('N15 ratio');
    f.Position(3:4)=[960,400];
end
...
ratioratio(history_ratio>0)=(history_ratio2(history_ratio>0)./history_ratio(history_ratio>0));
...
for index=1:5
    ...
    %scatter(1:224, history_ratio2(pools_sorted_indices(end-223:end),indices(index)), 150, 'o', 'MarkerEdgeColor',colors(index,:));
...
for aa_atom_i = 1:length(aa_atoms)
    ...
    %history_ratio2_aa(aa_atom_i, : ) = history_ratio2(current_atoms_idx,:)'*current_partitioning;
end
...
for index=1:5
    ...
    %scatter(1:length(aa_atoms), history_ratio2_aa(:,indices(index)), 150, 'o', 'MarkerEdgeColor',colors(index,:));
end
hold off
